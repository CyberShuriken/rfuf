...
If a run reports `step subfinder incomplete (status=failed exit_code=0)`, `step amass_enum incomplete (status=failed exit_code=0)`, `step jsmap_scrape incomplete (status=failed exit_code=0)`, or `step hidden_params_arjun incomplete (status=failed exit_code=0)`, update to RFUF v2.4.5 or later and rerun. The stage-health artifact map validates the actual producer outputs, materializes declared reports for legitimate zero-result stages, and ignores temporary redirect files that a stage explicitly removes. A genuinely empty result is recorded as `completed_empty`, not failed. RFUF v2.4.5 covers `scope_guard`, `jsmap_scrape`, and `hidden_params_arjun`, materializing `scope.json`, `js_assets.txt`, `js_endpoints.txt`, `jsmap_status.txt`, and `hidden_params.txt` when those stages complete without findings.
