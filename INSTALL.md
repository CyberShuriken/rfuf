...
If the network cannot register an OOB callback, RFUF continues without interactsh after the default 20-second wait. Use `-interactsh-timeout 0` or `-disable-interactsh` when OOB callbacks are unavailable or out of scope. Previous false stage-health errors—`subfinder incomplete (status=failed exit_code=0)` and `amass_enum incomplete (status=failed exit_code=0)`—are fixed in RFUF v2.4.2 by validating the actual producer outputs and creating empty artifacts when a tool legitimately returns no results. Such runs are recorded as `completed_empty`. RFUF v2.4.2 also materializes all `scope_guard` outputs before returning, preventing an empty exact-domain result from being reported as a failed stage.
